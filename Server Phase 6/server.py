"""
Tiny relay server for the Secure Voice project — Phase 6 (PRE-encryption).

Forwards bytes between two registered devices identified by phone number.
This is the plain, dumb-pipe version: no AES, no key fingerprint, no .wav
capture — just call-control plus transparent byte forwarding once GO is sent.
Use this with firmware built BEFORE Phase 7 (no encryption on the wire).
